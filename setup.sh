...
releasename=alfresco-content-services-installer-5.2.4-linux-x64.bin

# Change working dir to vagrant directory
cd /vagrant

# Install Alfresco using the key file
chmod 755 $releasename
...
cd /opt/alfresco
cp -Rv /vagrant/shared-classes/* tomcat/shared/classes/ 2>/dev/null
cp -Rv /vagrant/shared-lib/* tomcat/shared/lib/ 2>/dev/null
cp -Rv /vagrant/amps-repo/* amps/ 2>/dev/null
cp -Rv /vagrant/amps-share/* amps_share/ 2>/dev/null
